...
def test_tc_val_01(browser_config, test_case):
    """TC_Val_01: Verify registration fails with empty Full Name"""
    logging.info("TC_Val_01 Started..")
    logging.info(test_case["validation_cases"][0]["TC_Val_01"]["_comment"])
    ...
    # Get test data
    test_data = test_case["validation_cases"][0]["TC_Val_01"]
    ...
    try:
        ...
        capture_full_page_screenshot(driver, "TC_Val_01_Navigation_Error")
        pytest.fail(f"Navigation to Sign Up page failed: {e}")

    # 1. Enter EMPTY value of Full Name
    try:
        registration_page.enter_full_name(test_data["fullname"])
        logging.info("Empty Full Name entered successfully.")
    except Exception as e:
        logging.error(f"Full Name entry failed: {e}")
        capture_full_page_screenshot(driver, "TC_Val_01_FullName_Error")
    ...
    try:
        ...
        capture_full_page_screenshot(driver, "TC_Val_01_Email_Error")
    ...
    try:
        ...
        capture_full_page_screenshot(driver, "TC_Val_01_Password_Error")
    ...
    try:
        ...
        capture_full_page_screenshot(driver, "TC_Val_01_ConfirmPassword_Error")
        pytest.fail(f"Confirm Password entry failed: {e}")

    # Take screenshot before validation attempt
    capture_full_page_screenshot(driver, "TC_Val_01_Before_Submit")
    ...
    try:
        ...
        capture_full_page_screenshot(driver, "TC_Val_01_Submit_Error")
        pytest.fail(f"Sign Up button click failed: {e}")

    # Validate Error Message
    try:
        expected_error_message = test_data["expected_result"]
        actual_error_message = registration_page.get_validation_error_message("FullName")

        logging.info("Actual error message: {actual_error_message}")
        ...
        if actual_error_message and expected_error_message in actual_error_message:
            ...
            screenshot_path = capture_full_page_screenshot(driver, "TC_Val_01_Validation_Error")
            ...
            bug_report = generate_bug_report(
                ...
                test_case_name="TC_Val_01"
            )

            # Save bug report
            save_bug_report(bug_report, "TC_Val_01")
        ...
        capture_full_page_screenshot(driver, "TC_Val_01_Exception")
    ...
    try:
        ...
        if "/en" == current_url.lower():
            logging.error("Registration unexpectedly succeeded with invalid data")
            capture_full_page_screenshot(driver, "TC_Val_01_Unexpected_Success")
            pytest.fail("Registration unexpectedly succeeded with invalid data")
        else:
            logging.info("Registration correctly prevented with invalid Full Name")
    except Exception as e:
        logging.warning(f"Additional validation failed: {e}")

    logging.info("TC_Val_01 Completed..")